...
- Si després d'introduir el correu i la contrasenya el formulari torna a aparèixer amb el missatge “Sessió iniciada però no s'ha pogut carregar el perfil docent”, vol dir que Supabase ha acceptat les credencials però no pot accedir a la taula `profiles`.
  - Assegura't que has executat el contingut complet de `supabase/setup.sql` (en mode **Raw**).
...
### Funcionalitats Supabase actives

La integració Supabase manté el portal docent, les classes amb codi, les assignacions i el registre de resultats. Les partides Live s'han retirat de la interfície i de l'esquema principal, de manera que una instal·lació nova només necessita executar `supabase/setup.sql`.
