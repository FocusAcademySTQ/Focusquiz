    ...
    const MODULES = [
      ...
      { id:'eq1',   name:'Equacions lineals', desc:'Resol ax + b = c.', badge:'Àlgebra', gen: genEq1 },
    ];
    ...
    function startQuizFromExisting(moduleId, options, questions){
      ...
      session = {
        ...
        startedAt: Date.now(),
        secondsLeft: 0,
        questions: questions.map(q=>({ type:q.type, text:q.text, title:q.title, html:q.html, answer:q.answer, meta:q.meta, numeric:q.numeric, piCoef:q.piCoef })),
        options: options || {}
      };
    ...
    function redoWrongs(){
      ...
      const qs = session.wrongs.map(w=>({type:w.type, text:w.text, title:w.title, html:w.html, answer:w.answer, meta:w.meta, numeric:w.numeric, piCoef:w.piCoef}));
      closeModal();
