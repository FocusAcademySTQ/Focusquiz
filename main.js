...
function htmlToReportText(value){
  ...
  template.content.querySelectorAll('.math-frac[aria-label], [role="img"][aria-label]').forEach((node) => {
    const label = node.getAttribute('aria-label') || '';
    const fraction = label.replace(/\s+sobre\s+/i, '/').replace(/\s+/g, '');
    if (fraction) {
      node.replaceWith(document.createTextNode(fraction));
    }
  });
    ...
    wrongs.forEach((wrong) => {
      ...
      if (current.examples.length < 2) {
        current.examples.push({
          question: htmlToReportText(wrong.text) || 'Pregunta no disponible',
          user: htmlToReportText(wrong.user) || '—',
          correct: htmlToReportText(wrong.correct || fmtAns(wrong.answer)),
...
function init(){
  ...
  if (chip) {
    ...
    if (!chip.dataset.resultsBound) {
      chip.addEventListener('click', () => showView('results'));
      chip.dataset.resultsBound = 'true';
    }
  }
