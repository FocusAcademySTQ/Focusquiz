...
function renderProfileWarning() {
  ...
  if (state.session && !state.profile) {
    ...
    const troubleshooting =
      ...
      "1. SQL Editor → botó **Raw** de `supabase/setup.sql`, enganxa'l sencer i prem **Run**.\n" +
      "2. Verifica que RLS estigui actiu amb `select n.nspname, c.relname, c.relrowsecurity from pg_class c join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relname in ('profiles','classes','assignments','submissions') order by c.relname;`.\n" +
