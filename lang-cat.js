...
(function(){
 ...
 // 🔹 Inicialment buit
let BANK_MORFO_SUBJECTE = [];

// 🔹 Funció per carregar el JSON
async function loadMorfoQuestions() {
  try {
    const response = await fetch("bank_morfo.json");
    if (!response.ok) throw new Error("Error carregant el JSON");
    BANK_MORFO_SUBJECTE = await response.json();
    console.log("✅ Preguntes carregades:", BANK_MORFO_SUBJECTE.length);
  } catch (err) {
    console.error("❌ Error carregant banc Subjecte-Predicat:", err);
  }
}

// 🔹 Executem la càrrega quan arrenca la pàgina
loadMorfoQuestions();
